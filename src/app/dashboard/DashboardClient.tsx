...
function DashboardInner({ user }: { user: any }) {
  ...
  const [recapStats, setRecapStats] = useState<{ totalSongsPlayed: number; totalVotesCast: number; peakGuestCount: number; durationMinutes: number; roomCode: string; roomName: string } | null>(null);
  const [playlistExport, setPlaylistExport] = useState<{ loading: boolean; url: string | null; error: string | null }>({ loading: false, url: null, error: null });
  ...
  const closeRoom = async () => {
    ...
    if (res.ok) {
      ...
      const data = await res.json();
      if (data.stats) {
        setRecapStats({ ...data.stats, roomCode: code, roomName: activeRoom.name });
        setPlaylistExport({ loading: false, url: null, error: null });
      }
        ...
        {recapStats && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-6">
            <div className="bg-bg-card border border-border rounded-2xl p-6 max-w-sm w-full space-y-5">
              ...
              {recapStats.totalSongsPlayed > 0 && (
                playlistExport.url ? (
                  <a
                    href={playlistExport.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full py-2.5 bg-[#1DB954] text-black font-semibold rounded-xl flex items-center justify-center gap-2 transition-colors hover:bg-[#1ed760]"
                  >
                  ...
                  <button
                    onClick={async () => {
                      setPlaylistExport({ loading: true, url: null, error: null });
                      try {
                        const res = await fetch(`/api/rooms/${recapStats.roomCode}/export-playlist`, { method: "POST" });
                        const data = await res.json();
                        if (res.ok && data.playlistUrl) {
                          setPlaylistExport({ loading: false, url: data.playlistUrl, error: null });
                        } else {
                          setPlaylistExport({ loading: false, url: null, error: data.error || "Export failed" });
                        }
                      } catch {
                        setPlaylistExport({ loading: false, url: null, error: "Export failed" });
                      }
                    }}
                    disabled={playlistExport.loading}
                    className="w-full py-2.5 bg-[#1DB954] hover:bg-[#1ed760] disabled:opacity-50 text-black font-semibold rounded-xl flex items-center justify-center gap-2 transition-colors"
                  >
                    {playlistExport.loading ? (
                      <>
              ...
              {playlistExport.error && (
                <p className="text-red-400 text-xs text-center">{playlistExport.error}</p>
              )}
              <button
  ...
  if (view === "create") {
    ...
    return (
      <div className="flex flex-col min-h-dvh max-w-2xl lg:max-w-3xl mx-auto overflow-hidden overscroll-none select-none safe-top lg:min-h-0 lg:my-6 lg:bg-white/[0.03] lg:backdrop-blur-xl lg:border lg:border-white/[0.06] lg:rounded-3xl lg:shadow-[0_8px_32px_rgba(0,0,0,0.3)] lg:max-h-[calc(100dvh-3rem)]" style={{ height: 'var(--app-height, 100dvh)' }}>
        {/* Sticky Header */}
                ...
                {requests.map((req) => (
                  ...
                  >
                    {(() => {
                      ...
                      return (
                    <button
                      onClick={() => openPreview(req.spotifyUri, req.trackName, req.artistName, req.albumArt || undefined)}
