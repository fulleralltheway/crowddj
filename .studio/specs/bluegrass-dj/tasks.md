...
- [x] **T0 — Create worktree on `feature/bluegrass-dj`** ✓ 2026-04-28
...
- [x] **T1 — Add `BluegrassSession` model + push migration** ✓ 2026-04-28 — schema deployed to Neon prod, `prisma generate` clean, table visible with `@@index([userId, isActive])`.
...
- [x] **T2 — `/api/bluegrass/sessions` (POST create, GET active)** ✓ 2026-04-28
...
- [x] **T3 — `/api/bluegrass/sessions/[id]` (GET, PATCH, DELETE — DELETE is the kill switch)** ✓ 2026-04-28
...
- [x] **T4 — `/play`, `/state`, `/devices` endpoints** ✓ 2026-04-28 — added `startPlaybackContext` helper to `src/lib/spotify.ts` (additive, non-breaking).
...
- [x] **T5 — Fade endpoints: `/fade-pause`, `/fade-resume`, `/fade-skip`** ✓ 2026-04-28
...
- [x] **T6 — `/api/cron/sync-bluegrass` (threshold detection)**
...
- [x] **T7 — `/api/cron/bluegrass-fade-transition` (server-side fade + skip)**
...
- [x] **T8 — Add `vercel.json` cron entry**
...
- [x] **T9 — Unit tests for threshold detection**
...
- [x] **T10 — Extend `socket-server.ts` with parallel session machinery**
...
- [x] **T11 — Client-polling threshold fallback in `BluegrassClient.tsx`**
...
- [x] **T12 — `/bluegrass/page.tsx` server component (auth-gated)**
...
- [x] **T13 — `BluegrassClient.tsx` (full UI)**
...
- [x] **T14 — PWA assets (manifest + 4 icons + sw.js extension)**
